(function () {
  ...
  let paused = false;
  buttons.$pause.on("click", (e) => {
    paused = !paused;
    if (paused) {
      buttons.$pause.html('<i class="fa-solid fa-play"></i>');
      buttons.$pause.addClass("active");
      clearInterval(updateInterval);
    } else {
      buttons.$pause.html('<i class="fa-solid fa-pause"></i>');
      buttons.$pause.removeClass("active");
      updateInterval = setInterval(tick, config.tickspeed);
    }
  });
  buttons.$slower.on("click", (e) => {
    setTickspeed(config.tickspeed * 2);
  });
  buttons.$faster.on("click", (e) => {
    setTickspeed(config.tickspeed / 2);
  });
  buttons.$restart.on("click", (e) => {
    snowGrid = makeGrid(canvas.width, canvas.height);
  });
  buttons.$snowAdd.on("click", (e) => {
    config.snowflakes += 10;
    if (config.snowflakes > config.maxSnowflakes) config.snowflakes = config.maxSnowflakes;
  });
  buttons.$snowSub.on("click", (e) => {
    config.snowflakes -= 10;
    if (config.snowflakes < config.minSnowflakes) config.snowflakes = config.minSnowflakes;
  });
  buttons.$download.on("click", (e) => {
    /** @type {HTMLCanvasElement} */
